            ...
            for b in np.arange(a, 23):
                for arm_b in ['p', 'q']:
                    for aneu_type_a, aneu_type_b in aneu_types:
                        ...
                        s_sample.loc["{}{}{}-{}{}{}".format(a, arm_a, aneu_type_a, b, arm_b, aneu_type_b)]= pval_method(mat, a, arm_a, aneu_type_a, b, arm_b, aneu_type_b)
...
def generate_bg_dists(aneu_types, chr_interaction, file_name, n_start, n_end, original, pval_method, p_factor, file_suffix, use_cache):
    ...
    if use_cache and os.path.exists(os.path.join(constants.CACHE_FOLDER, cache_file_format.format(file_name))):
        return pd.read_csv(os.path.join(constants.CACHE_FOLDER, cache_file_format.format(file_name)), sep='\t', index_col=0)
...
def plot_dists(original, bg, interactions=None, file_name="", p_factor=10, file_suffix=None, use_cache=1):
    ...
    if len(interactions)>9:
        ...
        interaction_sets=[interactions[a*9:min((a+1)*9, len(interactions))] for a in np.arange(len(interactions)/9 +1)]
    else:
    ...
    df_emp_pvals.to_csv(os.path.join(constants.OUTPUT_FOLDER, "emp_pvals_summary_{}_{}.tsv".format(file_suffix, file_name)), sep='\t')
...
def plot_dist(enrichment_score, bg, interaction, p, ax=None):
    ...
    ax.annotate(str('real score'), xy=(enrichment_score, 1), xytext=(enrichment_score, 10), color='red', xycoords='data',
                textcoords='offset points',
                bbox=dict(boxstyle="round", fc="0.8"),
                arrowprops=dict(arrowstyle="->"))
    ax.set_yscale('log')
    ax.set_title("events of {}\nempirical p-value: {} (co-occurring score: {})".format(interaction, "%.2E" % p, "%.2f" % enrichment_score))

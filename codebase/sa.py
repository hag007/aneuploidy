...
def permute_matrix(args):
    ...
    for a in range(sa_factor / 10):
        original = permute_slice(original, seed=a)

    prev_e_score = -1
    cur_e_score = None
    
    for a in range(sa_factor):
        original_c = permute_slice(np.array(original), seed=sa_factor+a)       
        ...
        if step or e_score < prev_e_score:
            original = original_c
            cur_e_score = e_score
        # if a % 100000 == 0:
        #     print "{}	cur_e_score: {}".format(n_iteration, cur_e_score)
